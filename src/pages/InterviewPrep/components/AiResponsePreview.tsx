    ...
    return (
        <div className="max-w-4xl mx-auto">
            ...
            p({ children }) {
                        ...
                        h2({ children }) {
                            return <h2 className="text-xl font-bold mt-6 mb-2=3">{children}</h2>;
                        },
                        h3({ children }) {
                            return <h3 className="text-lg font-bold mt-5 mb-2">{children}</h3>;
                        },
                        h4({ children }) {
                            return <h4 className="text-md font-bold mt-4 mb-2">{children}</h4>;
                        },
                        a({ children, href }) {
                            return (
                                <a
                        ...
                        table({ children }) {
                            return (
                                <table className="over-x-auto my-4">
                                    {children}
                                </table>
                            );
                        },
                        ...
                        tbody({ children }) {
                            return (
                                <tbody className="devide-y devide-gray-200">
                                    {children}
                                </tbody>
                            );
                        },
  ...
  <div className="relative my-6 rounded-lg bg-gray-50 border border-gray-200">
    <div className="flex items-center justify-between px-4 py-2 border-b border-gray-200">
      ...
      >
        ...
        {copied && (
          <span className="absolute -top-8 right-0 bg-back text-white text-xs rounded-md px-2 py-1 opacity-80 group-hover:opacity-100 transition">Copied!</span>
        )}
      </button>
    </div>
    <SyntaxHighlighter
